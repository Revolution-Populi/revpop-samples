...
const config = {
    ...
    asset_name: "RVP",
    asset: null,
    chain_id: process.env.CHAIN_ID || "147f0c14427dc6c28ecc977229e5067cf3608cb6d16f79aa39888845ee4ca2bf",
    balance_address: "RVPBrWn6V3fn1KdCZRo4DVmrMq7MpTtic5CW"
};
...
const CONTENT_CARD_ID_START = '1.' + ChainTypes.object_type.content_card + '.0';
const CONTENT_CARD_V2_ID_START = '1.' + ChainTypes.object_type.content_card_v2 + '.0';
const PERMISSION_ID_START = '1.' + ChainTypes.object_type.permission + '.0';
const CONTENT_VOTE_ID_START = '1.' + ChainTypes.object_type.content_vote + '.0';
...
function fee(amount) {
    return { amount, asset_id: config.asset.id };
}
...
async function get_all_content_cards(subject_account) {
    return await get_all_objects('get_content_cards_v2', subject_account, CONTENT_CARD_V2_ID_START);
}
...
async function get_content_card(content_id) {
    return await db_exec('get_content_card_v2_by_id', content_id);
}
...
async function create_content_card(account, content) {
    ...
    return await transaction(account.key, 'content_card_v2_create', {
        fee: no_fee(),
        ...
        storage_data: content.storage_data,
    });
}
...
async function update_content_card(account, content) {
    ...
    content.description = content.description || '';
    content.key = content.key || make_content_key();
    return await transaction(account.key, 'content_card_v2_update', {
        fee: no_fee(),
        subject_account: account.acc.id,
        hash: content.hash,
        url: content.url,
...
async function remove_content_card(subject_account, content_card_id) {
    return await transaction(subject_account.key, 'content_card_v2_remove', {
        fee: no_fee(),
...
async function transfer(from, to, amount) {
    await transaction(from.key, "transfer", {
        // fee: no_fee(),
        from: from.acc.id,
        to: to.acc.id,
        amount: {amount, asset_id: config.asset.id}
    });
}
